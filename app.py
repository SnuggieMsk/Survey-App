...
# Comment out matplotlib if you're having issues installing it
# You can uncomment these later once you have matplotlib installed
try:
    import matplotlib
...
# Add context processor for template utility functions
@app.context_processor
def utility_processor():
    return {
        'now': datetime.datetime.now,
        'format_date': lambda date, format='%Y-%m-%d': date.strftime(format) if date else '',
        'format_currency': lambda amount: f"${amount:.2f}" if amount is not None else '$0.00'
    }
...
def view_survey(survey_id):
    ...
    # Check if viewer is the creator (for UI customization)
    is_creator = survey['creator_ip'] == request.remote_addr
...
def update_question(question_id):
    try:
        ...
        with DatabaseConnection() as conn:
            ...
            question = conn.execute('SELECT q.*, s.creator_ip, s.id as survey_id FROM questions q JOIN surveys s ON q.survey_id = s.id WHERE q.id = ?', (question_id,)).fetchone()
            
            if not question or (question['creator_ip'] and question['creator_ip'] != request.remote_addr):
...
def upload_option_image(option_id):
    try:
        with DatabaseConnection() as conn:
            ...
            if not option or (option['creator_ip'] and option['creator_ip'] != request.remote_addr):
                return jsonify({'error': 'Unauthorized'}), 403
    ...
    try:
        ...
        with DatabaseConnection() as conn:
            ...
            for answer in answers:
                ...
                question = conn.execute('SELECT question_type, required FROM questions WHERE id = ?', (question_id,)).fetchone()
                ...
                if question_type == 'multiple-choice' or question_type == 'image-choice':
                    ...
                    if text_answer or not question['required']:
                        conn.execute(
        ...
        return jsonify({'error': 'An error occurred while submitting your response'}), 500
